...
import { getRepliedComments, addComment, deleteComment} from '../../utils/database-api';
...
const CommentItem = (props) => {
    const { id, content, username, postDate, level = 0, repliedTitle, commentId } = props;
    ...
    const toggleDeleteForm = () => {
        setDeleteFormVisible((prev) => !prev);  // Переключить видимость формы
    };
    ...
    return (
        <div>
            <ul className={styles.list}>
                <div className={styles.card} style={dynamicStyle}>
                    <li className={styles.item}>
                        {repliedTitle && <div style={{color : 'grey', fontSize: 'small', marginBottom : '10px'}}> <MdReply size={15} style={{transform: 'scaleX(-1)'}}/> {repliedTitle} <hr style={{marginTop : '5px', marginRight : '50px'}}></hr></div>}
                            ...
                            <span className={styles.date}>{formattedDate}</span>
                            {
                                <div className={styles.deleteButtonContainer}>
                                    ...
                                    {isDeleteFormVisible && (
                                            ...
                                            : 
                                            <div className={styles.deleteConfirm} style={{ bottom : '0px'}}>
                                                <button
                                                        className={styles.cancelDeleteButton}
                                                        onClick={toggleDeleteForm}
                                                    >
            ...
            <div className={styles.replyFormContainer} style={{marginLeft : dynamicStyle.marginLeft, maxWidth: Math.min((800 - level*40), 600)}}>
                    {isReplyFormVisible && (
                        <div className={styles.expandedForm}>
                            ...
                            <div className={styles.buttonContainer}>
                                <button onClick={toggleReplyButton} className={styles.cancelButton}>Cancel</button>
                                <button onClick={handleCreateComment} className={styles.submitButton}>Comment</button>
                            </div>
                            </div>
                    )}
            </div>
